(function(window,undefined){
    ...
    var Sliders = function(container, slider, count, options){
        ...
        this.direction = bVertical ? 'top' : 'left';//方向
    ...
    Sliders.prototype = {
        ...
        getTween:function(){
            switch(this.options.tween){
                case 0:
                    return Tween.Quart.easeOut; break;
                case 1:
                    return Tween.Back.easeOut; break;
                default:
        ...
        Run: function(index){
            ...
            //设置参数
            var self=this;
            this._target = -Math.abs(this.change) * (this.index = index);//目标位置；
            this._t = 0;
            this._b = parseInt(this._slider.position()[this.options.vertical ? 'top' : 'left']);//当前位置；
            this._c = this._target - this._b;//需要移动的距离；
            this.onStart();
            this.Move();
        ...
        Move: function(){
            ...
            if (this._c && this._t < this.duration){
                ...
                this.MoveTo(this._target);//移动到指定位置，防止动画产生的误差；
